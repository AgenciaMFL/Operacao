...
| `assets/img/trust-card-bg-mobile.webp` | Mesmo card, telas ≤720px (`<picture>` source) — imagem enviada pelo usuário (800x921, cantos arredondados já embutidos via alpha), com faixa vazia reservada no topo (~53% da altura, confirmado por variância de pixel linha a linha) onde o texto é sobreposto, em vez do card cobrindo a imagem inteira; mesma técnica de `.trust-band` no desktop |
