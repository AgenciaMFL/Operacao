...
| `assets/img/trust-card-bg-mobile.webp` | Mesmo card, telas ≤720px (`<picture>` source) — segunda versão enviada pelo usuário (800x1031, cantos arredondados já embutidos via alpha; substitui uma primeira versão 800x921 que exigia crop e deixava uma linha de corte visível no topo do mockup), com faixa vazia embutida (~54% da altura) grande o bastante pra não precisar de nenhum crop — exibida na proporção nativa abaixo do texto (fluxo normal, mesmo padrão de `.nr1-panel__mockup`/`.climate-cta__mockup`), não mais sobreposta como fundo do card inteiro |
