...
| `assets/img/climate-cta-mockup-mobile.webp` | Mesma banda, telas ≤720px (`.climate-cta__mockup`) — recorte apertado (630x344) do export do Figma node 2981:36 "Mobile" (enviado pelo usuário, originalmente 800x713 com uma faixa vazia grande demais para o mobile), fundo sólido #106b61 idêntico a `--color-teal-dark` (emenda com o card invisível, sem crop/máscara) e bordas retas; exibida como imagem flutuante após o texto, no mesmo padrão de `.nr1-panel__mockup` (texto em fluxo normal no topo, mockup abaixo, sem esticar a imagem para cobrir o card inteiro) |
...
| `assets/img/trust-card-bg.webp` | Fundo full-bleed do card "Estivemos dos dois lados da mesa..." (fechamento da seção 05) — enviada diretamente pelo usuário, ícones metálicos + gradiente teal já embutidos. Usada em telas ≥721px (fundo cobrindo o card inteiro) |
| `assets/img/trust-card-bg-mobile.webp` | Mesmo card, telas ≤720px (`<picture>` source) — recorte (800x561, ~16% de margem) de uma imagem enviada pelo usuário (800x1031, substitui uma primeira versão 800x921 que exigia crop e deixava uma linha de corte visível no topo do mockup); a versão 800x1031 na íntegra tinha ~54% de faixa vazia, o que deixava um vão enorme entre o texto e o mockup em qualquer largura comum de celular — exibida na proporção nativa abaixo do texto (fluxo normal, mesmo padrão de `.nr1-panel__mockup`/`.climate-cta__mockup`), não mais sobreposta como fundo do card inteiro |
| `assets/img/trust-band-bg.webp` | Fundo da seção 06 "Sigilo e LGPD" (Figma node 2951:995) — mãos segurando um tablet com um escudo, enviada diretamente pelo usuário, já exportada na mesma proporção do frame (2000x785 ≈ 2561x1005 do Figma). Em telas ≥1025px é exibida por inteiro (sem crop) como fundo full-bleed, com o texto posicionado por cima na faixa vazia reservada pelo próprio Figma; em telas 721-1024px (layout empilhado, texto acima da imagem) a faixa vazia é recortada via `object-fit:cover` já que nada mais fica sobreposto a ela |
| `assets/img/trust-band-bg-mobile-v2.webp` | Mesma seção, telas ≤720px (`<picture>` source) — recorte (800x411, margem justa) de uma segunda versão enviada pelo usuário (800x1031, mãos+tablet+escudo, faixa vazia no topo ~70% da altura), substitui `trust-band-bg.webp` só nesse tier. O crop via CSS (`object-fit:cover`) usado inicialmente deixava uma linha de emenda visível; agora é um arquivo pré-recortado exibido na proporção nativa, mesmo padrão de `.trust-card__bg`/`.climate-cta__mockup` |
