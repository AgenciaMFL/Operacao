...
| `assets/img/trust-band-bg.webp` | Fundo da seção 06 "Sigilo e LGPD" — mãos segurando um tablet com um escudo, enviada diretamente pelo usuário; título+lede+cards ficam empilhados ACIMA da imagem (fluxo normal, não sobrepostos), e a faixa de gradiente vazio no topo do arquivo original é recortada via `object-fit:cover` (sem esticar, só corta — não perde qualidade) para o tablet ficar logo abaixo do texto em qualquer largura |
